/**
 * @file explore.jsx
 * @description Interactive civic issues map exploration screen - Neon Civic OS style.
 */

import React, { useEffect, useContext, useMemo, useState, useRef } from 'react';
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, ActivityIndicator, Image, Modal, Platform } from 'react-native';
...
export default function ExploreScreen() {
  ...
  // Calculate dynamic offsets for absolute map controls above bottom tab bar and below status bar
  ...
  useEffect(() => {
    if (filteredReports.length > 0 && mapRef.current) {
      const coords = filteredReports
        .map((r) => (r.location && r.location.latitude && r.location.longitude ? {
          ...
          <View className="items-center">
            ...
            >
              ...
              <View className="flex-col">
                <Text className="text-textDark font-bold text-[11px]">
                  {cat.name}
                </Text>
          ...
          >
            <View style={styles.calloutContainer}>
              <Text style={styles.calloutTitle} numberOfLines={1}>
                {String(report.title || 'Civic Report')}
              </Text>
  ...
  }, [filteredReports, selectedReport, router]);
